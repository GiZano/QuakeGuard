  ...
  for (;;) {
    ...
    if (pressDur >= 1000 && pressDur < RESET_HOLD_MS) {
      ...
      String portalHtml =
          ...
          "  <div style='background:#e9ecef; padding:10px; border-radius:4px; font-family:monospace; word-break:break-all; font-weight:bold; color:#d63384; font-size:1em; user-select:all; -webkit-user-select:all; cursor:text;' id='qg-pubkey2'>"
          + String(pub_hex2.data()) +
          "  </div>"
          "  <button onclick=\"(function(){var t=document.getElementById('qg-pubkey2').innerText; if(navigator.clipboard&&window.isSecureContext){navigator.clipboard.writeText(t).then(function(){alert('Public key copied!')}).catch(function(){fallback()});}else{fallback();} function fallback(){var ta=document.createElement('textarea');ta.value=t;ta.style.position='fixed';ta.style.opacity='0';document.body.appendChild(ta);ta.select();try{if(document.execCommand('copy')){alert('Public key copied!');}else{throw 'e';}}catch(e){prompt('Copy manually (Ctrl+C):',t);} document.body.removeChild(ta);}})()\" style='margin-top:8px; padding:6px 14px; background:#198754; color:#fff; border:none; border-radius:6px; font-size:0.82em;'>&#128203; Copy Key</button>"
...
void setup() {
  ...
  String customHtml =
      ...
      "  <div style='background:#e9ecef; padding:10px; border-radius:4px; font-family:monospace; word-break:break-all; font-weight:bold; color:#d63384; font-size:1em; user-select:all; -webkit-user-select:all; cursor:text;' id='qg-pubkey'>"
      + String(pub_hex.data()) +
      "  </div>"
      "  <button onclick=\"(function(){var t=document.getElementById('qg-pubkey').innerText; var ok=false; if(navigator.clipboard&&window.isSecureContext){navigator.clipboard.writeText(t).then(function(){alert('Public key copied!')}).catch(function(){fallback()});}else{fallback();} function fallback(){var ta=document.createElement('textarea');ta.value=t;ta.style.position='fixed';ta.style.opacity='0';document.body.appendChild(ta);ta.select();try{if(document.execCommand('copy')){alert('Public key copied!'); ok=true;}else{throw 'exec failed';}}catch(e){prompt('Copy manually (Ctrl+C):',t);} document.body.removeChild(ta);}})()\" style='margin-top:8px; padding:6px 14px; background:#198754; color:#fff; border:none; border-radius:6px; font-size:0.82em;'>&#128203; Copy Key</button>"
