...
- *Outbox Pattern:* Only the first worker process that successfully acquires the Redis lock will persist the `Alert` to PostgreSQL and publish the JSON payload to the `quake_alerts` Redis Pub/Sub channel (see @fig-alert-delivery-1, @fig-alert-delivery-2, and @fig-ai-report-prop for the complete delivery sequence).


  #align(center)[
    #figure(
      image("assets/fig06_1-backend-alert-propagation.svg", width: 100%),
  ...
  #align(center)[
    #figure(
      image("assets/fig06_2-mobile-alert-propagation.svg", width: 100%),
...
- *Dashboard-as-Code:* The full "Mission Control" dashboard is provisioned via a JSON file mounted into Grafana's provisioning directory.
  ...
  #align(center)[
    #figure(
      image("assets/fig06_3-ai-report-propagation.svg", width: 100%),
